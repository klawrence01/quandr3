// /app/u/[id]/followers/_FollowersInner.tsx
"use client";
// @ts-nocheck
...
export default function FollowersInner() {
  ...
  // list items: { follower_id, created_at, profile: {...} }
    ...
    async function load() {
      ...
      try {
        ...
        const ids = Array.from(
          new Set(relRows.map((r: any) => r?.follower_id).filter(Boolean))
        );
        ...
        const merged = relRows.map((r: any) => ({
          ...r,
          profile: pMap[r.follower_id] || { id: r.follower_id },
        }));

        if (!ignore) setRows(merged);
      } catch (e: any) {
        if (!ignore) setErr(e?.message || "Could not load followers.");
      } finally {
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <section className="mt-6 rounded-[28px] border bg-white p-6 shadow-sm">
          {loading ? (
            ...
            <div className="text-sm text-slate-700">No followers yet.</div>
          ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="truncate text-xs text-slate-500">
                          Curioso ID: {shortId(p?.id || r.follower_id)}
                        </div>
                      </div>
