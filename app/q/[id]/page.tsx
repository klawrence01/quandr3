...
function getCreatorId(qRow: any) {
  return (
    qRow?.author_id || qRow?.user_id || qRow?.creator_id || qRow?.created_by || null
  );
}
...
export default function Quandr3DetailPage() {
  ...
  /* ✅ UPDATE #1: Trust DB status first, fallback to computed */
  const status = useMemo(() => {
    const db = (q?.status || "").toLowerCase().trim();
    if (db === "open" || db === "awaiting_user" || db === "resolved") return db;
  ...
  /* ✅ UPDATE #3: Better Options header copy by state */
  const optionsTitle = useMemo(() => {
    if (status === "open") return "Pick your answer";
    if (status === "awaiting_user") return "Internet decided — Curioso deciding";
    return "Final decision";
  }, [status]);

  const optionsSub = useMemo(() => {
    if (status === "open") return "Vote while it’s open. Then add a short reason (optional).";
    if (status === "awaiting_user") return "Voting has closed. Results are in while the Curioso makes the final call.";
    return "The Curioso has made the final decision. Learn from the outcome and the reasons.";
  }, [status]);
        ...
        <section className="mt-7 rounded-[28px] border bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-sm text-slate-600">{optionsSub}</div>
            </div>
          </div>
                    ...
                    <div className="grid gap-0 grid-cols-[140px_1fr]">
                      ...
                      <div className="p-5">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {/* ✅ UPDATE #2: Correct badge by state */}
                          {status === "resolved" ? (
